...
export async function generateImagePrompt(title, category = "", tags = []) {
  try {
    if (!title || title.trim().length === 0) {
      throw new Error("Title is required to generate image prompt");
    }

    const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash-exp" });

    const prompt = `
Generate a detailed, creative image prompt for an AI image generator based on this blog post:

Title: "${title}"
${category ? `Category: ${category}` : ""}
${tags.length > 0 ? `Tags: ${tags.join(", ")}` : ""}

Create a compelling image prompt that would generate a perfect featured image for this blog post. The prompt should be:
...
Format: Return only the image prompt text, no additional explanation.
`;
    ...
    if (!imagePrompt || imagePrompt.trim().length < 10) {
      throw new Error("Generated image prompt is too short");
    }
    ...
    return {
      success: false,
      error: error.message || "Failed to generate image prompt. Please try again.",
    };
  }
}